...
- **Name strip layout (default):** Fixed bottom strip with **`pn`**; strip height is now fixed at **`72px`** and strip font size at **`20px`**. Strip background = `dc` at 50% with black (**`#000000`** fallback). Strip mode still uses **`np.st.ts`** for the remaining typography fields (`fw`, `c`, `ls`, `sh`, `st`, `ta`) plus **`maxWidthPercent`** default **80%** — **`np.x/y/w/h` ignored** for layout only. **`nl`** defaults to `"strip"`; **`ar`** includes strip; **`ip`** % stays on the background band (see schema docs).
